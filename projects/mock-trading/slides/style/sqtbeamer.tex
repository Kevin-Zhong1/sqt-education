...
\usepackage{colortbl}          % cell colors in tables
...
\addtobeamertemplate{footline}{}{%
  \begin{tikzpicture}[remember picture, overlay]
    % Bottom left logo
    % Include if needed
    % \node[anchor=south west, inner sep=0.3cm, xshift=-0.2cm, yshift=-0.2cm] at (current page.south west) {
    %   \includegraphics[height=0.5cm]{style/____.png}
    % };
    % Bottom right logo
    \node[anchor=south east, inner sep=0.3cm, xshift=0.2cm, yshift=-0.3cm] at (current page.south east) {
      \includegraphics[height=0.7cm]{style/logo_sqt.png}
    };
  \end{tikzpicture}
}
